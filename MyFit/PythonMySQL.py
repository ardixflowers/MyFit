...
def Formulario():
    ...
    try:
        ...
        tree1 = ttk.Treeview(groupBox,columns=("Id","Nombre","Apellido","Partida","Precio","Fecha"),show="headings",height=11,)
        tree1.column("# 1",width=30,anchor=CENTER)
        tree1.heading("# 1",text="Id")
        tree1.column("# 2",width=95,anchor=CENTER)
        tree1.heading("# 2",text="Nombre")
        tree1.column("# 3",width=95,anchor=CENTER)
        tree1.heading("# 3",text="Apellido")
        tree1.column("# 4",width=95,anchor=CENTER)
        tree1.heading("# 4",text="Partida")
        tree1.column("# 5",width=70,anchor=CENTER)
        tree1.heading("# 5",text="Precio")
        tree1.column("# 6",width=80,anchor=CENTER)
        tree1.heading("# 6",text="Fecha")
        ...
        tree1.bind("<<TreeviewSelect>>",seleccionarRegistro)
        #Se termina la tabla
        tree1.pack()

        #Panel tabla 2
        groupBox = LabelFrame(base,text="Lista de Stock",padx=5,pady=5,)
        groupBox.grid(row=0,column=3,padx=10,pady=10)

        tree2 = ttk.Treeview(groupBox,columns=("Id","Nombre","Apellido","Partida","Precio"),show="headings",height=11,)
        tree2.column("# 1",width=30,anchor=CENTER)
        tree2.heading("# 1",text="Id")
        tree2.column("# 2",width=95,anchor=CENTER)
        tree2.heading("# 2",text="Nombre")
        tree2.column("# 3",width=95,anchor=CENTER)
        tree2.heading("# 3",text="Apellido")
        tree2.column("# 4",width=95,anchor=CENTER)
        tree2.heading("# 4",text="Partida")
...
def seleccionarRegistro(event):
    try:
        itemSeleccionado = tree1.focus()

        if itemSeleccionado:
            #obtiene el valor de la comlumna
            values = tree1.item(itemSeleccionado)["values"]
